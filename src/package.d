...
version(D_BetterC){
    ...
    extern(C) void _memset64(ulong* uints, ulong value, size_t length){
        for (size_t i = 0; i < length; ++i)
            uints[i] = value;
    }
...
void _main_impl()/+@safe pure nothrow @nogc+/{


	import std.stdio;
	{
		BasicString!char str = cast(byte)45;
		writeln("'", str[], "'");
		assert(str == "45", str[]);
...
version(D_BetterC)
    extern(C)void main(){
        ...
        import basic_string.unittesting;
        version(unittest)unittest_impl!Mallocator();
    }
else
    void main(){
        _main_impl();
...
//I hate it when people compare Freddie Mercury to god, i mean he's cool and all, but he's no Freddie Mercury. (с)﻿
